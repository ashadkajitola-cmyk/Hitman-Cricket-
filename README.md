...
<body>
    ...
    <div id="loginOverlay">
        <div class="login-box">
            <h3>📱 Phone Login</h3>
            <p style="font-size: 13px; color: #666;">App mein enter karne ke liye apna mobile number dalein.</p>
            <input type="tel" id="loginPhoneInput" placeholder="Mobile Number (e.g. 9876543210)" maxlength="10">
            <button type="button" class="action-btn" id="loginSubmitBtn" style="margin-top: 12px;">Set / Login</button>
            <p id="loginErrorMsg" style="color: red; font-size: 12px; margin-top: 8px;"></p>
        </div>
    </div>
    ...
    <div id="dashboardSection">
        ...
        <div id="tabWallet" class="tab-content hidden">
            <div class="card" style="background: #e0f2f1;">
                ...
                <p style="font-size: 12px; text-align: center; color: #666;">Phone/User ID: <span id="myUserIdDisplay" style="font-family: monospace; background: #fff; padding: 2px 4px; border-radius: 3px;">Loading...</span></p>
                ...
                <button type="button" id="logoutPhoneBtn" class="logout-btn" style="background: #d32f2f; width: 100%; margin-top: 10px;">Change Number / Logout</button>
            </div>
...
<script>
    ...
    function handlePhoneLogin() {
        ...
        let deviceNumbers = JSON.parse(localStorage.getItem('cricket_device_numbers') || '[]');

        if(!deviceNumbers.includes(phoneInput)) {
            if(deviceNumbers.length >= 3) {
                errorMsg.innerText = "Is device par maximum 3 numbers pehle hi register ho chuke hain!";
                return;
            }
            deviceNumbers.push(phoneInput);
            localStorage.setItem('cricket_device_numbers', JSON.stringify(deviceNumbers));
        }
    ...
    function initUserSession(phoneNum) {
        ...
        userRef.once('value', (snapshot) => {
            // Fix: Check exact number for 5000 points bonus
            ...
            if(!snapshot.exists()) {
                ...
                // If user exists but points are missing/0 and it's the specific number, fix it automatically
    ...
    function checkAdminAccess() {
        ...
        // Allow admin access directly for the owner number or active subscription holders
        if(myPhone === "9569981484" || (currentUserSessionData && currentUserSessionData.subExpiry && currentUserSessionData.subExpiry > currentTime)) {
    ...
    function buySubscription(cost, planName, extraDays, extraMins) {
        ...
        userRef.once('value', (snapshot) => {
            ...
            if(userData && userData.points !== undefined) {
                currentPoints = parseInt(userData.points) || 0;
            }
            ...
            let currentTime = Date.now();
            let subExpiry = currentTime;

            if(extraDays && extraDays > 0) {
                subExpiry = currentTime + (extraDays * 24 * 60 * 60 * 1000);
            } else if(extraMins && extraMins > 0) {
                subExpiry = currentTime + (extraMins * 60 * 1000);
            }
            ...
            // Fix: Use update instead of set to preserve other fields securely, or pass complete object properly
            userRef.update(updatedData, (error) => {
                if(!error) {
                    currentUserSessionData = updatedData;
                    document.getElementById('userPoints').innerText = newPoints;
                    updateSubscriptionStatusUI(updatedData);
    ...
    function updateSubscriptionStatusUI(userData) {
        ...
        if(myPhone === "9569981484") {
            subStatusElem.innerText = "Status: Admin Owner (Full Access)";
